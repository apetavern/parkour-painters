...
PS
{
	...
	float4 MainPs( PixelInput i ) : SV_Target0
	{
		...
		float4 local13 = Tex2DS( g_twindowmap, g_sSampler0, local2 );
		float2 local14 = CalculateViewportUv( i.vPositionSs.xy );
		float3 local15 = CalculatePositionToCameraDirWs( i.vPositionWithOffsetWs.xyz + g_vHighPrecisionLightingOffsetWs.xyz );
		float local16 = g_flFakeReflectMultiplier;
		float3 local17 = local15 * float3( local16, local16, local16 );
		float3 local18 = float3( local14.xy, 0 ) * local17;
		float4 local19 = Tex2DS( g_treflect, g_sSampler0, local18.xy );
		float4 local20 = local13 * local19;
		float4 local21 = local12 + local20;
		float4 local22 = Tex2DS( g_tNormal, g_sSampler0, local2 );
		float3 local23 = TransformNormal( i, DecodeNormal( local22.xyz ) );
		float4 local24 = Tex2DS( g_tRoughness, g_sSampler0, local2 );
		float4 local25 = Tex2DS( g_tMetallic, g_sSampler0, local2 );

		m.Albedo = local21.xyz;
		m.Opacity = 1;
		m.Normal = local23;
		m.Roughness = local24.x;
		m.Metalness = local25.x;
		m.AmbientOcclusion = 1;
