...
PS
{
	...
	CreateTexture2DWithoutSampler( g_twindowmap ) < Channel( RGBA, Box( windowmap ), Srgb ); OutputFormat( DXT5 ); SrgbRead( True ); >;
	CreateTexture2DWithoutSampler( g_treflect ) < Channel( RGBA, Box( reflect ), Srgb ); OutputFormat( DXT5 ); SrgbRead( True ); >;
	...
	float4 MainPs( PixelInput i ) : SV_Target0
	{
		...
		float4 local11 = Tex2DS( g_twindowmap, g_sSampler0, local2 );
		float2 local12 = CalculateViewportUv( i.vPositionSs.xy );
		float3 local13 = CalculatePositionToCameraDirWs( i.vPositionWithOffsetWs.xyz + g_vHighPrecisionLightingOffsetWs.xyz );
		float local14 = g_flFakeReflectMultiplier;
		float3 local15 = local13 * float3( local14, local14, local14 );
		float3 local16 = float3( local12.xy, 0 ) * local15;
		float4 local17 = Tex2DS( g_treflect, g_sSampler0, local16.xy );
		float4 local18 = local11 * local17;
		float4 local19 = local10 + local18;
		float4 local20 = Tex2DS( g_tNormal, g_sSampler0, local2 );
		float3 local21 = TransformNormal( i, DecodeNormal( local20.xyz ) );
		float4 local22 = Tex2DS( g_tRoughness, g_sSampler0, local2 );
		float4 local23 = Tex2DS( g_tMetallic, g_sSampler0, local2 );

		m.Albedo = local19.xyz;
		m.Opacity = 1;
		m.Normal = local21;
		m.Roughness = local22.x;
		m.Metalness = local23.x;
		m.AmbientOcclusion = 1;
